               ...
               <?php  }else if($row['po_status'] == 'รับสินค้าแล้ว'){ ?>
                ...
                <?php }else if($row['po_status'] == 'ยกเลิก'){ ?>
                    ...
                    <?php }else{ ?>
                        ...
                        <?php }?>
            <td scope="row">
            <a href="?page=<?= $_GET['page'] ?>&function=detail&po_RefNo=<?= $row['po_RefNo'] ?>" class="btn btn-primary">ข้อมูล</a>  
            <?php if($row['po_status'] != 'สั่งแล้ว' &&  $row['po_status'] != 'รับสินค้าแล้ว' &&  $row['po_status'] != 'ยกเลิก') { ?>
            <a onclick="return confirm('ต้องการสั่งซื้อหรือไม่ ??')" href="?page=<?= $_GET['page'] ?>&function=delete&po_RefNo=<?= $row['po_RefNo'] ?>" class="btn btn-danger">ลบ</a>  
            <?php } ?>
            </td>
        </tr>
        <?php } ?>
