...
$sql = ("SELECT DATE_FORMAT(sales_create, '%d') AS sales_create, #เปลี่ยนฟอแมตวันที่จาก ป-ด-ว ให้เป็น ด-ป
													SUM(product_total) as product_total #หาผลรวมของราคาขาย
													FROM view_sales_detail 
													GROUP BY DATE_FORMAT(sales_create, '%d') #จัดกลุ่มข้อมูลจาก ด-ป
													ORDER BY DATE_FORMAT(sales_create, '%d') DESC #เรียงลำดับข้อมูลจากมากไปน้อย
													");
