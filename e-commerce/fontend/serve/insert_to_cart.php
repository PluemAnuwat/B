      ...
      if(empty($cus_id) ) {
          ...
          if($n > 0 ) { 
              ...
              if($show_sub_prod_id == 0 ){  //เช็คว่าเลือกยัง
                  ...
                  $s1 =  "SELECT * FROM akksofttech_subproduct_one  WHERE  prod_id = '".$prod_id."' AND sprod_id = '".$show_sub_prod_id."'" ;   $q1 = mysqli_query ($conn , $s1 ) ;  $r1 = mysqli_fetch_array($q1) ;    $n1 = mysqli_num_rows($q1) ;  
                      ...
                      if($show_sub_prodone_id == 0 ){ //เช็คมีเลือกยัง
                          ...
                          $coms = "YESSS1" ;
                              ...
                              if($nn == "" ){
                                  ...
                                  // มีสินค้านี้ยัง
                                  $secc = "SELECT * FROM akksofttech_cart WHERE  `prod_id` = '".$prod_id."' AND sprod_id = '".$show_sub_prod_id."' AND sprodone_id = '".$show_sub_prodone_id."'  AND sto_id = '".$show_sto_id."' " ;
                                  $recc = mysqli_query($conn ,$secc);
                                  $qecc = mysqli_fetch_array($recc);
                                  $necc = mysqli_num_rows($recc); 
   
                                      if($necc > 0 ){
   
                                          $secc1 = "SELECT * FROM akksofttech_cart WHERE  `prod_id` = '".$prod_id."' AND sprod_id = '".$show_sub_prod_id."' AND sprodone_id = '".$show_sub_prodone_id."'  AND sto_id = '".$show_sto_id."' " ;
                                          $recc1 = mysqli_query($conn ,$secc1);
                                          $qecc11 = mysqli_fetch_array($recc1);
   
                                          if($show_sto_id == $qecc1['sto_id']){
   
                                              $coms = "STO" ; 
          
                                              $see1 = "UPDATE `akksofttech_cart` SET  `quantity` = `quantity` + $show_quantity  WHERE  `prod_id` = '".$prod_id."' AND sprod_id = '".$show_sub_prod_id."' AND sprodone_id = '".$show_sub_prodone_id."'   ";
                                              $ree1 = mysqli_query($conn ,$see1);
                                              if($ree1){
                                                  $coms = 'YESUP2';
                                              }else{
                                          ...
                                          $secc11 = "SELECT * FROM akksofttech_cart WHERE  cus_id = '".$cus_id."' " ;
                                          $recc11 = mysqli_query($conn ,$secc11);
                                          $qecc11 = mysqli_fetch_array($recc11);
   
                                          if($show_sto_id == $qecc11['sto_id']){
